...
class Events_API_Render {
	...
	public function display_events_shortcode() {
		...
		// Добавим вычисление цвета текста для каждого события и форматирование месяца
		foreach ($events as &$event) {
			$event['text_color'] = $this->get_text_color($event['background_color']);
			$event['start_month'] = date_i18n('M', strtotime($event['start_date'])); // Форматируем месяц
		}
        ...
        <div class="container">
            <div class="row">
                <div class="coming-soon-events" data-events='<?php echo json_encode($events); ?>' data-quantity='<?php echo $quantity; ?>'>
	...
	public function display_event($event) {
		...
		$start_month = date_i18n('M', strtotime($event['start_date'])); // Форматируем месяц
		$text_color = $this->get_text_color($event['background_color']);
		ob_start();
		?>
        <div class="event" style="background: #<?php echo esc_attr($event['background_color']); ?>; color: #<?php echo esc_attr($text_color); ?>">
            <div class="date">
	...
	private function get_text_color($hexcolor) {
		$r = hexdec(substr($hexcolor, 0, 2));
		$g = hexdec(substr($hexcolor, 2, 2));
		$b = hexdec(substr($hexcolor, 4, 2));
		$brightness = ($r * 299 + $g * 587 + $b * 114) / 1000;
		return ($brightness > 180) ? '202124' : 'ffffff';
	}
}
